#include <iostream>
#include <vector>
#include <queue>
#include <climits>
#include <thread>
#include <chrono>
#include <algorithm>
#include <cstring>

#ifdef _WIN32
#include <windows.h>
void setCursor(int row, int col) {
    COORD coord;
    coord.X = (SHORT)col;
    coord.Y = (SHORT)row;
    SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
}
void hideCursor() {
    CONSOLE_CURSOR_INFO info;
    info.dwSize = 1;
    info.bVisible = FALSE;
    SetConsoleCursorInfo(GetStdHandle(STD_OUTPUT_HANDLE), &info);
}
void showCursor() {
    CONSOLE_CURSOR_INFO info;
    info.dwSize = 1;
    info.bVisible = TRUE;
    SetConsoleCursorInfo(GetStdHandle(STD_OUTPUT_HANDLE), &info);
}
void clearScreen() { system("cls"); }
#else
void setCursor(int row, int col) { printf("\033[%d;%dH", row + 1, col + 1); }
void hideCursor() { printf("\033[?25l"); fflush(stdout); }
void showCursor() { printf("\033[?25h"); fflush(stdout); }
void clearScreen() { printf("\033[2J\033[H"); fflush(stdout); }
#endif

const int ROWS = 20;
const int COLS = 40;

bool wall[ROWS][COLS];

struct Node {
    int dist, row, col;
    bool operator>(const Node& o) const { return dist > o.dist; }
};

std::vector<std::pair<int, int>> dijkstra(int sR, int sC, int eR, int eC) {
    const int INF = INT_MAX;
    std::vector<std::vector<int>> dist(ROWS, std::vector<int>(COLS, INF));
    std::vector<std::vector<std::pair<int, int>>> prev(ROWS, std::vector<std::pair<int, int>>(COLS, std::make_pair(-1, -1)));
    std::priority_queue<Node, std::vector<Node>, std::greater<Node>> pq;
    dist[sR][sC] = 0;
    pq.push({ 0, sR, sC });
    int dr[] = { -1, 1, 0, 0 };
    int dc[] = { 0, 0, -1, 1 };

    while (!pq.empty()) {
        Node cur = pq.top(); pq.pop();
        int r = cur.row, c = cur.col;
        if (cur.dist > dist[r][c]) continue;
        if (r == eR && c == eC) break;
        for (int d = 0; d < 4; d++) {
            int nr = r + dr[d], nc = c + dc[d];
            if (nr < 0 || nr >= ROWS || nc < 0 || nc >= COLS) continue;
            if (wall[nr][nc]) continue;
            int nd = dist[r][c] + 1;
            if (nd < dist[nr][nc]) {
                dist[nr][nc] = nd;
                prev[nr][nc] = std::make_pair(r, c);
                pq.push({ nd, nr, nc });
            }
        }
    }

    std::vector<std::pair<int, int>> path;
    if (dist[eR][eC] == INF) return path;
    int r = eR, c = eC;
    while (r != -1 && c != -1) {
        path.push_back(std::make_pair(r, c));
        int pr = prev[r][c].first;
        int pc = prev[r][c].second;
        r = pr; c = pc;
    }
    std::reverse(path.begin(), path.end());
    return path;
}

void drawGrid(int pR, int pC, int sR, int sC, int eR, int eC, bool arrived) {
    for (int r = 0; r < ROWS; r++) {
        setCursor(r, 0);
        for (int c = 0; c < COLS; c++) {
            if (r == pR && c == pC) {
                std::cout << "x ";
            }
            else if (r == eR && c == eC && !arrived) {
                std::cout << "B ";
            }
            else if (r == sR && c == sC && (r != pR || c != pC)) {
                std::cout << "A ";
            }
            else if (wall[r][c]) {
                std::cout << "* ";
            }
            else {
                std::cout << "- ";
            }
        }
    }
    std::cout.flush();
}

void generateWalls(int sR, int sC, int eR, int eC) {
    for (int r = 0; r < 15; r++) {
        int wc = 10;
        if ((r == sR && wc == sC) || (r == eR && wc == eC)) continue;
        wall[r][wc] = true;
    }
}

int main() {
    int sR, sC, eR, eC;
    std::cout << "A redak: "; std::cin >> sR;
    std::cout << "A stupac: "; std::cin >> sC;
    std::cout << "B redak: "; std::cin >> eR;
    std::cout << "B stupac: "; std::cin >> eC;
    sR--; sC--; eR--; eC--;

    if (sR < 0 || sR >= ROWS || sC < 0 || sC >= COLS ||
        eR < 0 || eR >= ROWS || eC < 0 || eC >= COLS) {
        showCursor();
        std::cout << "Neispravne koordinate!\n";
        return 1;
    }

    memset(wall, false, sizeof(wall));
    generateWalls(sR, sC, eR, eC);

    std::vector<std::pair<int, int>> path = dijkstra(sR, sC, eR, eC);
    if (path.empty()) {
        showCursor();
        std::cout << "Ne postoji put od A do B!\n";
        return 1;
    }

    clearScreen();
    hideCursor();

    for (int i = 0; i < (int)path.size() - 1; i++) {
        drawGrid(path[i].first, path[i].second, sR, sC, eR, eC, false);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    drawGrid(eR, eC, sR, sC, eR, eC, true);
    setCursor(ROWS + 1, 0);

    return 0;
}